...
const deployer = {
  ...
  async publishContract(cont) {
    ...
    // Create folder "../contracts/" if not exist
    let frontendContractDir = path.join(__dirname, "../contracts");

    // libraries linking
    let libraries = {}
    let addressesPath = path.join(__dirname, "../contracts/addresses.json")
    ...
    fs.copyFileSync(
      ...
      path.join(__dirname, "../contracts/" + name + ".json") // destination
    );

    // check if addresses.json already exists
    let exists = fs.existsSync(path.join(__dirname, "../contracts/addresses.json"));
    ...
    let addressesFile = fs.readFileSync(path.join(__dirname, "../contracts/addresses.json"));
